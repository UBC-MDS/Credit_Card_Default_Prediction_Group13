...
We would also aim to a comparative study of the mainstream machine learning classification models to be able to identify how the best performing model assigns weights to the various model features.
...
The dataset consists of 30000 instances, with each consists of 23 attributes and a target.
...
1 = high school; 2 = university; 3 = graduate school; 4 = others.\
...
Upon our first look at the data, we found some features contained ambiguous categories.
...
We also see there is a slight increase in the default ratio in the middle to late age range of our clients.
...
This is also expected as higher payments in recent month will result in less likelihood of a client defaulting.

Below is the table of the coefficients of all the features.

```{r}
kable(read.csv("../data/results/feature_coefficient.csv")) %>%
  kable_classic_2(full_width = F)
```
