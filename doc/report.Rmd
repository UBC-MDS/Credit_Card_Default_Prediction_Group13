...
There were `r sum(as.numeric(model_score[7,2]), as.numeric(model_score[8,2]))` incorrect predictions, either a customer will default on their payment when they have not or a customer will not default when they have.
...
There is class imbalance in our data, with `r round(as.numeric(model_score[1,2]) / sum(as.numeric(model_score[1:2,2])), 3) * 100`% of examples as class 0 and `r round(as.numeric(model_score[2,2]) / sum(as.numeric(model_score[1:2,2])), 3) * 100`% as class 1.
...
From the cross validation scores for each of these models, the mean validation score returned highest for the Random Forest Classifier.
...
We then performed hyperparameter optimization for the Logistic Regression model to find our optimum hyperparamater `C` value as `r round(as.numeric(model_score[3,2]), 3)` and `r model_score[4,2]`.
Using our model with optimized hyperparameters, the f1 validation score returned was `r round(as.numeric(model_score[5,2]), 2)`.

## Results

We used our optimized Logistic Regression model on the test data of `r sum(as.integer(model_score[6:9,2]))` clients.
The f1 score on the test data was `r round(as.numeric(model_score[10,2]), 2)`.
Below are the regression coefficients for our features.

```{r fig.align='center'}
feat_table = read.csv("../results/model/feature_coefficient.csv") %>%
  select(c(Feature,Coefficient)) 

kable(feat_table, caption = "Table 2. Feature Coefficients") %>%
  kable_classic_2(full_width = F) 
```

Our strongest positively correlated feature was `PAY_0`: the default history of the client.
...
`BILL_AMT3` and `BILL_AMT2` were also strong positive coefficients.
...
Also, `PAY_AMT1` and `PAY_AMT2` are strong negatively correlated variables for our model.
...
Our model made `r sum(as.numeric(model_score[6,2]), as.numeric(model_score[9,2]))` correct predictions for our clients, out of `r sum(as.integer(model_score[6:9,2]))` (`r round((sum(as.numeric(model_score[6,2]), as.numeric(model_score[9,2]))) / (sum(as.numeric(model_score[6:9,2]))), 3) * 100`%).
...
We predicted `r model_score[8,2]` clients would not default and make their payment when in fact, they would not.
...
On the other hand, we made `r model_score[7,2]` false predictions on clients and predicted they would default, when they will not.
